...
def login_in():
	...
	loginbutton1=Button(login_menu,command=login_check,text="Login",height=1,width=9)
...
def Main_Menu():
	...
	#Bunch of labels and their positioning
	status = Label(base,text=("Time and date logged in: " + current_time),bd=1,relief=SUNKEN,anchor=W)
	orionLabel=Label(base, text="Orion",bg='#ffe500',font=("Times", "70","bold","italic","underline"),fg="blue")
...
def borrow_check():
	...
	if (bbook in bookborrow):
		...
		if bookborrow[Avail] == str("Unavailable\n"):
			...
			tkMessageBox.showinfo("Borrow","The book you have selected has been successfully borrow please return it on:" +'\n'+ str(enddate) )
...
def shop_in():
	...
	global shop_entry5
	global shop_menu
	#popup messagebox to inform user about the discount! :D
	mbox=Tk()
	mbox.withdraw()
	...
	shop_menu.attributes("-topmost", True)
	...
	shop_button1=Button(shop_menu,text="Purchase",command=shop_in_check)
...
def search_in():
	...
	search_button=Button(search_menu,text="Search",command=search_check)
...
def search_check():
	...
	if (search_word in booklist):
		...
		tkMessageBox.showinfo("Searh","It is in our database!")
...
def feedback_in():
	...
	button1=Button(feedback_menu, text="Submit feedback",command=feedback_check)
